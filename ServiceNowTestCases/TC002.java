...
public class TC002 {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		driver.findElementByXPath("//select[@id='incident.urgency']").click();
		driver.findElementByXPath("(//option[contains(text(),'1 - High')])[2]").click();

		driver.findElementByXPath("//select[@id='incident.state']").click();
		driver.findElementByXPath("//option[contains(text(),'In Progress')]").click();

		driver.findElementByXPath("//button[@id='sysverb_update_bottom']").click();

		// Verify the priority and state
		String verify = driver.findElementByXPath("//td[contains(text(),'3 - Moderate')]").getText();
		if (verify.contains("Moderate")) {
			System.out.println("Verified the Priority");
		} else {
			System.out.println("Not Verified");
		}
		String verify1 = driver.findElementByXPath("//td[contains(text(),'In Progress')]").getText();
		if (verify1.contains("Progress")) {
			System.out.println("Verified the State");
		} else {
			System.out.println("Not Verified");
		}
